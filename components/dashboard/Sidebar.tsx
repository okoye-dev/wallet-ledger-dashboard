...
const Sidebar = ({ isOpen = false, onClose }: SidebarProps) => {
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-black/10 backdrop-blur-sm z-40 lg:hidden transition-all duration-300 ease-in-out"
          onClick={onClose}
          style={{
            opacity: isOpen ? 1 : 0,
          }}
      ...
      <aside
        className={cn(
          ...
          "fixed lg:top-4 top-header z-[99] left-0 h-[calc(100vh-var(--header-height))] w-64 bg-surface shadow-lg transition-all",
          // Mobile: slide in/out animation
          "transform duration-300 ease-in-out",
          // Desktop: always visible, static positioning
          "lg:fixed lg:shadow-none lg:transform-none lg:h-full",
